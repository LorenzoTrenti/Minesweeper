...
def centra_testo(text, color, flag, size):
    if flag==1:
        ...
        textSurface=FONT2.set_underline(True)
        textSurface=FONT2.render(text, True, color)
    elif flag==3:
        FONT3 = pg.font.SysFont('Comic Sans MS', WIDTH//20, bold=True)
...
def menu_iniziale():
    ...
    for j in range(4):
        if j==0:
            modalità_sup, modalità_rect = centra_testo(modalità, LIGHT_GREEN, 3, 0)
            modalità_rect.center=(WIDTH/2, HEIGHT/3 + i)
            screen.blit(modalità_sup, modalità_rect)
        elif j==1:
            facile_sup, facile_rect = centra_testo(facile, "green", 2, 0)
            facile_rect.center=(WIDTH/2, HEIGHT/3 + i)
            screen.blit(facile_sup, facile_rect)
            # pg.draw.rect(screen, DARK_GREEN, facile_rect, 1)
        elif j==2:
            medio_sup, medio_rect = centra_testo(medio, "yellow", 2, 0)
            medio_rect.center=(WIDTH/2, HEIGHT/3 + i)
            screen.blit(medio_sup, medio_rect)
            # pg.draw.rect(screen, DARK_GREEN, medio_rect, 1)
        elif j==3:
            difficile_sup, difficile_rect = centra_testo(difficile, "red", 2, 0)
            difficile_rect.center=(WIDTH/2, HEIGHT/3 + i)
            screen.blit(difficile_sup, difficile_rect)
            # pg.draw.rect(screen, DARK_GREEN, difficile_rect, 1)
        i+=WIDTH/18
    instructions_sup, instructions_rect = centra_testo("left click: dig | right click: flag", LIGHT_GREEN, 3, 0)
    instructions_rect.center=(WIDTH/2, HEIGHT - HEIGHT/10)
    screen.blit(instructions_sup, instructions_rect)
...
def stampa_campo(size, rows, cols, controllo, tabella):
            ...
            if controllo[i][j]==1:
                ...
                if tabella[i][j]!=0 and tabella[i][j]!=BOMB:
                    if tabella[i][j]==1:
                        ...
                        color="red"
                    val_sup, val_rect = centra_testo(str(tabella[i][j]), color, 4, size)
                ...
                if (i+j)%2==0:
                    ...
                    pg.draw.rect(screen, LIGHT_GREEN, prato)
                if controllo[i][j]==2:
    ...
    # pg.draw.rect(screen, "white", back_rect, 1)
...
def controllo_vittoria(controllo, tabella, rows, cols):
    ...
    return vittoria
def hai_perso():
    lose_sup, lose_rect = centra_testo(lose, COLOR_LOSE, 3, 50)
    lose_rect.center=(WIDTH-(distanza_width/2), HEIGHT/2)
    screen.blit(lose_sup, lose_rect)

def hai_vinto():
    victory_sup, victory_rect = centra_testo(victory, COLOR_WIN, 3, 50)
    victory_rect.center=(WIDTH-(distanza_width/2), HEIGHT/2)
